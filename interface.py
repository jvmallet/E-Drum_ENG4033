...
serial = Serial("COM7", baudrate=31250)
...
def draw_grid(clicks, beat, actives):
    ...
    clap_text = label_font.render('Tom', True, colors[actives[4]])
    screen.blit(clap_text, (30, 430))
    tom_text = label_font.render('Ride', True, colors[actives[5]])
...
def play_notes():
    ...
    for i in range(len(clicked)):
        if clicked[i][active_beat] == 1 and active_list[i] == 1:
            if i == 0:
                nota(22, volume)
                serial.write(f"led 1 {beat_length}\n".encode("UTF-8"))
                time.sleep(0.005)
            if i == 1:
                nota(38, volume)
                serial.write(f"led 2 {beat_length}\n".encode("UTF-8"))
                time.sleep(0.005)
            if i == 2:
                nota(36, volume)
                serial.write(f"led 3 {beat_length}\n".encode("UTF-8"))
                time.sleep(0.005)
            if i == 3:
                nota(55, volume)
                serial.write(f"led 4 {beat_length}\n".encode("UTF-8"))
                time.sleep(0.005)
            if i == 4:
                nota(45, volume)
                serial.write(f"led 5 {beat_length}\n".encode("UTF-8"))
                time.sleep(0.005)
            if i == 5:
                nota(51, volume)
                serial.write(f"led 6 {beat_length}\n".encode("UTF-8"))
                time.sleep(0.005)


def le_serial():
    while 1:
        texto = serial.readline().decode().strip()
        if texto != "":
            print("arduino mandou:", texto)
...
thread = threading.Thread(target=le_serial)
thread.start()
